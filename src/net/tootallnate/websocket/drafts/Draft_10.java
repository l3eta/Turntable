...
public class Draft_10 extends Draft {
	...
	@Override
	public HandshakeState acceptHandshakeAsClient(Handshakedata request,
			Handshakedata response) throws Exception {
	...
	public ByteBuffer createBinaryFrame(Framedata framedata) {
		...
		int sizebytes = mes.length == 0 ? 0 : mes.length <= 125 ? 1
				: mes.length <= 65535 ? 2 : 4;// 4 bytes length currently not
												// useable
	...
	public Framedata translateSingleFrame(ByteBuffer buffer, int foff,
			int available) throws IncompleteException {
		...
		if (MASK) {
			byte[] maskskey = ByteBuffer.allocate(4).put(b, maskskeystart, 4)
					.array();
